# Core Features
- [x] Playlist (Create, Edit, Delete)
- [x] Add/Remove Songs from Playlist
- [x] Reorder Playlist (Drag & Drop)
- [x] Import/Export M3U Playlist
- [x] Now Playing Queue
- [x] Add to Queue
- [x] Play Next
- [x] Remove from Queue
- [x] Reorder Queue
- [x] Clear Queue
- [x] Mini Player
- [x] Multi Selection
- [x] Batch Actions
- [x] Refresh Music Library

# Audio Features
- [x] Sleep Timer
- [x] Equalizer
- [x] Bass Boost
- [x] Virtualizer
- [x] Loudness Enhancer
- [x] Reverb
- [ ] Crossfade
- [ ] Gapless Playback
- [x] Fade In / Fade Out
- [ ] ReplayGain Support

# Music Library
- [x] Folder View
- [x] Recently Added
- [x] Most Played
- [x] Never Played
- [x] Smart Playlists
- [x] Hide Short Audio
- [x] Hide WhatsApp Audio
- [x] Hide Telegram Audio
- [x] Exclude Specific Folders

# Metadata
- [x] Embedded Lyrics Support
- [x] Online Lyrics Support
- [x] Edit Song Metadata
- [x] Edit Album Artwork
- [x] Download Album Artwork
- [x] Automatic Artwork Detection

# User Experience
- [x] Material You Support
- [x] Dynamic Color (Android 12+)
- [x] AMOLED Black Theme
- [x] Blur Background
- [x] Glassmorphism UI
- [x] Animated Album Artwork
- [x] Rotating Album Artwork
- [x] Color Palette from Album Art

# Android Integration
- [x] Home Screen Widget
- [ ] Lock Screen Widget
- [ ] Android Auto
- [ ] Wear OS Support
- [ ] Chromecast
- [ ] Bluetooth Audio Output Selection
- [x] Quick Settings Tile
- [ ] Voice Search

# Advanced Features
- [x] Bookmark Playback Position
- [x] Resume Playback
- [x] Playback Speed
- [x] Pitch Control
- [ ] Volume Normalization
- [ ] Audio Focus Handling Improvements
- [x] Auto Pause on Headphone Disconnect
- [x] Resume on Bluetooth Reconnect

# Backup & Sync
- [x] Backup Settings
- [x] Backup Favorites
- [x] Backup Playlists
- [x] Restore Backup

# Statistics
- [x] Listening History
- [x] Listening Time
- [x] Play Count
- [x] Favorite Artists
- [x] Favorite Albums
- [x] Weekly Statistics
- [x] Monthly Statistics

Next: Lanjutkan Import/Export M3U, Edit & Download Album Artwork, Crossfade, Volume Normalization,  Android Integration 0/8 dan equalizer beserta efek audio lain